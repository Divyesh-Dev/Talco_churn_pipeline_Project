...
# Pages: Login | Dashboard | Predictor | At-Risk | Model | Admin
# ============================================================
...
st.set_page_config(page_title="Telecom Churn Analytics", page_icon="📡",
...
with st.sidebar:
    ...
    visible_pages = [p for p in all_pages if p not in admin_only or is_admin()]

    page = st.radio("Navigate", visible_pages)

    # Log page views
    log_event(user["username"], user["role"], "PAGE_VIEW", page)
    ...
    t_range=st.slider("Tenure (months)",int(df_all["tenure"].min()),int(df_all["tenure"].max()),
...
if page=="📊 Executive Dashboard":
    ...
    c1.metric("👥 Customers",f"{total:,}"); c2.metric("🚪 Churned",f"{churned:,}")
    c3.metric("📉 Churn Rate",f"{churn_r:.1f}%"); c4.metric("💸 Rev Lost/mo",f"Rs.{rev_loss:,.0f}")
    c5.metric("⏳ Avg Tenure",f"{df['tenure'].mean():.1f} mo"); c6.metric("🔴 High Risk",f"{high_risk:,}")
    st.divider()
    ...
    if st.button("🔮 Predict Churn & Get Offer",type="primary",use_container_width=True):
        ...
        fig=go.Figure(go.Indicator(mode="gauge+number",value=prob*100,
            ...
            gauge={"axis":{"range":[0,100]},"bar":{"color":color},
                   "steps":[{"range":[0,35],"color":"#d4edda"},{"range":[35,65],"color":"#fef3cd"},{"range":[65,100],"color":"#fde8e8"}]}))
...
# ══════════════════════════════════════════════════════════════
    ...
    if not HAS_ML: st.warning("Run `python main.py --no-load` to generate predictions."); st.stop()
    c1,c2,c3=st.columns(3)
...
# ══════════════════════════════════════════════════════════════
    ...
    if not metrics_data: st.warning("Run `python main.py --no-load` to train model."); st.stop()
    ...
    st.plotly_chart(fig,use_container_width=True)
    if fimp:
        st.divider(); st.markdown("### Top Churn Drivers")
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with col_del:
            ...
            with st.container(border=True):
                del_options = [u["username"] for u in users_list
                               if u["username"] != user["username"]]
                if del_options:
                    del_target = st.selectbox("Select user to delete", del_options)
    ...
    with tab2:
        st.markdown('<div class="section-hdr">Change a User\'s Password</div>',
                    unsafe_allow_html=True)
        with st.container(border=True):
            all_usernames = [u["username"] for u in get_all_users()]
            pwd_target    = st.selectbox("Select User", all_usernames, key="pwd_target")
            new_pwd1      = st.text_input("New Password",     type="password", key="pwd1")
            new_pwd2      = st.text_input("Confirm Password", type="password", key="pwd2")
    ...
    with tab3:
        ...
        if log_data:
            ...
            with lc1:
                event_opts = ["All"] + sorted(log_df["event"].unique().tolist())
                sel_event  = st.selectbox("Filter by Event", event_opts)
            with lc2:
                uname_opts = ["All"] + sorted(log_df["username"].unique().tolist())
                sel_uname  = st.selectbox("Filter by User", uname_opts)
            with lc3:
                n_rows = st.number_input("Show last N rows", 10, 1000, 100, step=10)

            filtered = log_df.copy()
            if sel_event != "All": filtered = filtered[filtered["event"]==sel_event]
            if sel_uname != "All": filtered = filtered[filtered["username"]==sel_uname]
            ...
            lk4.metric("👁️ Page Views",      len(log_df[log_df["event"]=="PAGE_VIEW"]))

            st.dataframe(filtered, use_container_width=True, height=380, hide_index=True)

            csv_log = log_df.to_csv(index=False).encode()
            st.download_button("⬇️ Download Full Log", csv_log,
                               "access_log.csv", "text/csv",
                               use_container_width=True)
        else:
            st.info("No log entries yet.")

st.caption("Divyesh Joshi | MC24097 | MCA-II Sem IV | IIMS Chinchwad, Pune | SPPU 2025-26")
